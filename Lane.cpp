...
#pragma omp critical (Lane_putAgent)
    {
        ...
        if (ita == _tmpAgents.end())
        {
            ...
            flag = true;
// putAgent
//if(agent->id()=="000017") cout <<"putAgent"<<endl;//uchida
        }
//by uchida 2016/1/19
//ここは必要だがコメントアウト
//        else
...
//======================================================================
bool Lane::eraseAgent(RoadOccupant* agent)
{
    bool flag = false;

        vector<RoadOccupant*>::iterator ita = find(_agents.begin(),
                                                   _agents.end(),
                                                   agent);
...
// eraseするつもりの車が_agentsに存在していない
// 本来はeraseしなくとも
// - 充電切のためputAgentしない
// - _tmpAgentsに追加されないためrenewAgentLineで_agentsにswapされない
// - 結果としてdeleteStrandedAgentすべき時には_agentに存在せず，
//   ObjManagerからdeleteすれば問題ない（この関数は不要）はずだが…
// →ここはちゃんと解析すべき15.11.12 by uchida）
//        else
//        {
//            cout << "eraseAgent : " << agent << endl;
//            assert(0);
//        }
...
//======================================================================
void Lane::addStrandedVehicles(Vehicle* strandedVehicle)
{
    _parent->addStrandedVehicles(strandedVehicle);
}
