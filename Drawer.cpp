...
void IntersectionDrawer::draw(const Intersection& inter) const
{
    ...
    if (GVManager::getFlag("VIS_CS_ID"))
    {
        drawCSId(inter);
    }
    if (GVManager::getFlag("VIS_CS_VALUE"))
    {
        drawChargeValue(inter);
    }
...
// by uchida 2016/5/23
//----------------------------------------------------------------------
void IntersectionDrawer::drawCSId(const Intersection& inter) const
{
    if (inter.isCS())
    {
        GLColor::setCSId();
        const AmuPoint center = inter.center();
        AutoGL_DrawString(center.x(), center.y(), 32,
                          inter.id().c_str());
    }
}
...
// by uchida 2016/5/23
//----------------------------------------------------------------------
void IntersectionDrawer::drawChargeValue(const Intersection& inter) const
{
        if (inter.isCS())
        {
        // TODO by uchida 2016/5/23
        // setCSValue()を設定
        GLColor::setCSId();
        const AmuPoint center = inter.center();
        AutoGL_DrawString(center.x(), center.y()-5, 32,
                          inter.charge().c_str());
        GLColor::setCSValue();
        double radius = inter.charged() / 50;
        AutoGL_DrawCircle3D(center.x(), center.y(), -1,
                            0, 0, 1, radius, 6);
    }
}
...
void VehicleDrawer::draw(const Vehicle& vehicle) const
{
    ...
    if (SOC != "")
    {
        AutoGL_DrawString(x, y-5, z+5, SOC.substr(0,3).c_str());
    }
