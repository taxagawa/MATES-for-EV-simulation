...
public:
    ...
    void addRoutingProbability(bool isUp, VehicleType type, double probability);
    ...
    double routingProbability(Intersection* inter, VehicleType type, std::string stopCS) const;
...
// by uchida 2017/2/7
public:
    // Sectionに対応するSubNodeの集合を作成する
    void setSubNode(Section* section);
private:
    // SubNodeの間隔
    double _subDistance;

    // SubNodeの集合
    vector<SubNode*> _SubNodes;
public:
    // SubNodeの集合を返す
