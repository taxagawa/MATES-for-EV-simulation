...
//#define TMP_CODE
    ...
    if (GVManager::getFlag("FLAG_OUTPUT_WAITING_LINE"))
    {
        ...
        if (TimeManager::time() % interval == 0)
        {
            ...
            {
                // 待ち行列長出力
                string fLog = outputDir + "re-route_search/normal_quad/waitingLine1000.csv";
                ...
                {
                    string pLog = outputDir + "re-route_search/normal_quad/predictionValue1000.csv";
                    ...
                    if (!outpLogFile.fail())
                    {
                        ...
                        if (GVManager::getNumeric("WAITING_TIME_APPROXIMATION_DEGREE") == 1 || GVManager::getNumeric("WAITING_TIME_APPROXIMATION_DEGREE") == 2 || GVManager::getNumeric("WAITING_TIME_APPROXIMATION_DEGREE") == 3)
                        {
                            ...
                            {
                                // 5分後の予測値
                                outpLogFile << csNodes[i]->returnApproximationWaitingTime(5) << ",";
                            }
                            for (int i = 0; i < csNodes.size(); i++)
                            {
                                // 10分後の予測値
                                outpLogFile << csNodes[i]->returnApproximationWaitingTime(10) << ",";
